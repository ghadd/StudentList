...
const std::string DB_PATH = "E:\\QtApps\\StudentsList\\.students_db";
...
QString getStudent(QString name, QString surname) {
    if (!containsStudent(name.toStdString(), surname.toStdString())) {
	...
	for (int i = 0; i < lines - 1; i++) {
        ...
        if (QString::fromStdString(_name) == name && QString::fromStdString(_surname) == surname) {
